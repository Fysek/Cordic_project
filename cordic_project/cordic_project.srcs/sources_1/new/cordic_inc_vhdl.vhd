	...
	begin
		if reset = '1' then
			...
			if enable = '1' then
				...
				inc_value <= inc_value + inc_value_reg;
			else
				inc_value <= (others => '0');
				inc_value_reg <= (others => '0');
			end if; 	
...
---angle in value is 17:0, here shall be add
---angle_in <= angle * 1024; //Value in fixed-point (12:10) angle is in radians 
